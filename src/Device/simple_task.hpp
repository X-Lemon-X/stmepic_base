...
class SimpleTask
{
 ...
 public:
   ...
   * @param task function that will be run in the task loop (is shouldn't block or be
   * infinite loop)
   ...
   * @param before_task_task function that will be run before the task starts only once.
   * It will also cause the task to stop if it fails.
   * @param stack_size size of the stack for the task
   * @param priority priority of the task
   * @param name name of the task
   * @param stop_after_start_failure if true, the task will stop if the before_task_task
   * fails, if false, the task will continue running even if the before_task_task fails
   * @return Status
   */
   ...
   * It will allow for before_task_task is executed successfully and allow for the main
   * task function to start. Why would you want to use this? If you want to make sure that
   * the task is started before doing something else mostly starting other tasks. This is
   * the way to do it.
   *
   * ### For example:
   *
   * You want encoder, imu and motor to all start before you start the main control loop.
   * But each of this device have their own startup time and you want to make sure that
   * they are all working before your robot crashes in to  a wall. Hence you can use this
   * function to wait for each task to start before the others are started.
