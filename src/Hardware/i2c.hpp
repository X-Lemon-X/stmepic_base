...
class I2cBase : public HardwareInterface
{
 public:
  /**
   * @brief Read data from the I2C device from memory in blocking mode with other tasks
   * beeing able to freely run in the meantime returns the data read from the device after
   * the read is done
   *
   ...
   */
  virtual Status read(uint16_t address,
                      uint16_t mem_address,
                      uint8_t* data,
                      uint16_t size,
                      uint16_t mem_size = 1,
                      uint16_t timeout_ms = 300) = 0;

  /**
   * @brief Write data to the I2C device in blocking mode with other tasks beeing able to
   * freelu run in the
   *
   * @param address the address of the I2C device in 7 bir format it will be automatically
   * by 1
...
{
 public:
  I2cMultiplexerChannel(std::shared_ptr<I2cBase> i2c,
                        uint8_t channel,
                        MultiplexerBase& multiplexer)
    : _channel(channel)
    , _i2c(i2c)
  ...
  Status read(uint16_t address,
              uint16_t mem_address,
              uint8_t* data,
              uint16_t size,
              uint16_t mem_size = 1,
              uint16_t timeout_ms = 300) override;
  Status write(uint16_t address,
               uint16_t mem_address,
               uint8_t* data,
               uint16_t size,
               uint16_t mem_size = 1,
...
/// with auto handling of the channel switching depending on the requested channel by the
/// driver using the I2C interface.
class I2cMultiplexerGpioID : public MultiplexerBase
{
 public:
  ...
  /// @param address_pin_1 the first address pin of the multiplexer address pin 1 is
  /// mandatory.
  ...
  static Result<std::shared_ptr<I2cMultiplexerGpioID>> Make(
    std::shared_ptr<I2cBase> i2c,
    uint8_t channels,
  ...
  /// Which should be passed to the device driver which is connected to the multiplexer on
  /// this specific channel.
  /// @param channel the channel number to which the device is connected
  /// @return the I2C interface for the specific channel this should be passed to the
  /// device driver which is connected to the multiplexer on the specific channel.
