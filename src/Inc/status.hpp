...
/**
 * @brief Macro for assigning a value from a result and resetting the device on error in a
 * single line.
 *
 */
...
/**
 * @brief Macro for assigning a value to a already exisiting veriable from a result and
 * resetting the device on error in a single line.
 *
 */
...
/**
 * @brief Macro for checking the result and resetting the device on error in a single
 * line.
 *
 */
...
class Status
{
 public:
  ...
  {
    return Status(
      StatusCode::ExpressionValidationError, msg, "ExpressionValidationError|");
  };
...
{
 public:
  /// @brief Create a new Result from status for clean return from functions when error
  /// occurs.
